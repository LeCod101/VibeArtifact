"""
BackendTranslator 模块 — 后端代码翻译器。

将 backend agent 输出的 BackendPlan 翻译为 IROperation 列表。

翻译规则：
1. 每个 FileSpec → create_node（类型 code）
2. props 包含 path、content、language
3. 文件间根据目录层级创建 depends_on 边
   （models → schemas → services → routes → main）
"""
...
# 合法的后端代码 language 值
VALID_BACKEND_LANGUAGES = frozenset({
    "python", "dockerfile", "txt", "toml",
})


def _classify_backend_file(path: str) -> str | None:
    """
    根据文件路径判断所属层级。

    - path: 文件路径（如 "backend/models/user.py"）
    - 返回: 层级名称（如 "models"）或 None
    """
    # 去除 "backend/" 前缀
    cleaned = path.replace("\\", "/")
    if cleaned.startswith("backend/"):
        cleaned = cleaned[len("backend/"):]

    # 特殊文件匹配
    if cleaned == "main.py":
        return "main"
    if cleaned == "config.py":
        return "config"
    if cleaned == "database.py":
        return "database"

    # 目录匹配
    first_dir = cleaned.split("/")[0] if "/" in cleaned else None
    if first_dir in _BACKEND_LAYER_ORDER:
        return first_dir

    return None


class BackendTranslator(BaseTranslator):
    """
    后端代码翻译器。

    将 BackendPlan（后端代码文件列表）翻译为 IR 操作列表。
    产生 code 类型节点，以及文件间的 depends_on 关联边。
    """

    def translate(self, high_level_output: BaseModel) -> TranslatorResult:
        """
        将 BackendPlan 翻译为 IROperation 列表。
        ...
        - high_level_output: BackendPlan 实例
        - 返回: TranslatorResult，包含创建节点和边的操作列表
        """
        if not isinstance(high_level_output, BackendPlan):
            return TranslatorResult(
                operations=[],
                warnings=[
                    "BackendTranslator 期望 BackendPlan 类型，"
                    f"实际收到 {type(high_level_output).__name__}"
                ],
            )

        plan: BackendPlan = high_level_output
        ...
        for file_spec in plan.files:
            ...
            if not normalized_path.startswith("backend/"):
                warnings.append(
                    f"文件路径 '{file_spec.path}' 不以 'backend/' 开头"
                )
            ...
            # language 校验
            if file_spec.language not in VALID_BACKEND_LANGUAGES:
                warnings.append(
                    f"文件 '{normalized_path}' 使用了非标准 language "
                    f"'{file_spec.language}'，"
                    f"建议使用：{', '.join(sorted(VALID_BACKEND_LANGUAGES))}"
                )
            ...
            # 归类到层级
            layer = _classify_backend_file(normalized_path)
        ...
        # ==============================
        # 依赖链: config → database → models → schemas → services → routes → main
        # 高层级的文件依赖低层级的文件
        sorted_layers = sorted(
            layer_to_op_indices.keys(),
            key=lambda x: _BACKEND_LAYER_ORDER.get(x, 99),
        )
